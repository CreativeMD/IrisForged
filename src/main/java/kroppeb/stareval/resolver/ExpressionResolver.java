...
public class ExpressionResolver {
    ...
    private Expression resolveExpressionInternal(Type targetType, ExpressionElement expression, boolean allowNonImplicit, boolean allowImplicit) {
        ...
        if (expression instanceof UnaryExpressionElement) {
            ...
            return this.resolveCallExpression(targetType, token.getId(), token.getArgs(), allowNonImplicit, allowImplicit);
            ...
            if (!allowImplicit) {
                log("[DEBUG] failed to resolve variable %s (of type %s) to type %s without implicit casts", name, type, targetType);
                return null;
            }
