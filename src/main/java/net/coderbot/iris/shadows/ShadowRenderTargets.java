...
public class ShadowRenderTargets {
     ...
     * 
     * @param index
     *            The index of the render target to get
    ...
    private void create(int index) {
        ...
        targets[index] = RenderTarget.builder().setDimensions(resolution, resolution).setInternalFormat(settings.getFormat()).setPixelFormat(settings.getFormat().getPixelFormat())
                .build();
    ...
    public void copyPreTranslucentDepth() {
        if (translucentDepthDirty) {
            ...
            IrisRenderSystem.blitFramebuffer(depthSourceFb.getId(), noTranslucentsDestFb.getId(), 0, 0, resolution, resolution, 0, 0, resolution, resolution,
                GL30C.GL_DEPTH_BUFFER_BIT, GL30C.GL_NEAREST);
        } else {
            DepthCopyStrategy.fastest(false).copy(depthSourceFb, mainDepth.getTextureId(), noTranslucentsDestFb, noTranslucents.getTextureId(), resolution, resolution);
        }
    }
