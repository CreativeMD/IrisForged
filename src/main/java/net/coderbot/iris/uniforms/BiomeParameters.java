...
public class BiomeParameters {
    ...
    public static void addBiomeUniforms(UniformHolder uniforms) {
        ...
        uniforms.uniform1i(PER_TICK, "biome", playerI(player -> biomeMap.getInt(player.level().getBiome(player.blockPosition()).unwrapKey().orElse(null)))).uniform1i(PER_TICK,
            "biome_category", playerI(player -> {
            ...
            })).uniform1f(PER_TICK, "rainfall", playerF(player -> ((ExtendedBiome) (Object) player.level().getBiome(player.blockPosition()).value()).getDownfall())).uniform1f(
                PER_TICK, "temperature", playerF(player -> player.level().getBiome(player.blockPosition()).value().getBaseTemperature()))
                
                .uniform1i(ONCE, "PPT_NONE", () -> 0).uniform1i(ONCE, "PPT_RAIN", () -> 1).uniform1i(ONCE, "PPT_SNOW", () -> 2)
