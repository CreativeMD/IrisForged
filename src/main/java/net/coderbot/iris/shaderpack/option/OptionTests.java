...
public class OptionTests {
    public static void main(String[] args) throws IOException {
        ...
        source.getBooleanOptions().forEach((index, option) -> {
            ...
            System.out.println("[" + StringUtils.leftPad(Integer.toString(index + 1), 4, ' ') + "] " + type + " | " + StringUtils.rightPad(option.getName(), 32,
                ' ') + " | " + StringUtils.leftPad(Boolean.toString(option.getDefaultValue()), 5, ' ') + " | " + option.getComment().orElse(""));
        });
        ...
        source.getStringOptions().forEach((index, option) -> {
            ...
            System.out.println("[" + StringUtils.leftPad(Integer.toString(index + 1), 4, ' ') + "] | " + type + " | " + StringUtils.rightPad(option.getName(), 32,
                ' ') + " | " + StringUtils.leftPad(option.getDefaultValue(), 8, ' ') + " | " + option.getAllowedValues());
            System.out.println("       " + option.getComment().orElse("(no comment)"));
        });
        
        System.out.println("Diagnostics:");
        source.getDiagnostics().forEach((index, diagnostic) -> {
            System.out.println("[" + StringUtils.leftPad(Integer.toString(index + 1), 4, ' ') + "] " + diagnostic);
        });
