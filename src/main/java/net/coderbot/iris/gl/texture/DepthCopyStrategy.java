...
public interface DepthCopyStrategy {
    ...
    class Gl20CopyTexture implements DepthCopyStrategy {
        ...
        public void copy(GlFramebuffer sourceFb, int sourceTexture, GlFramebuffer destFb, int destTexture, int width, int height) {
            ...
            IrisRenderSystem.copyTexSubImage2D(destTexture,
                // target
                GL20C.GL_TEXTURE_2D,
                // level
                0,
    ...
    class Gl30BlitFbCombinedDepthStencil implements DepthCopyStrategy {
        ...
        @Override
        public void copy(GlFramebuffer sourceFb, int sourceTexture, GlFramebuffer destFb, int destTexture, int width, int height) {
            IrisRenderSystem.blitFramebuffer(sourceFb.getId(), destFb.getId(), 0, 0, width, height, 0, 0, width, height, GL30C.GL_DEPTH_BUFFER_BIT | GL30C.GL_STENCIL_BUFFER_BIT,
                GL30C.GL_NEAREST);
        }
    }
    ...
    class Gl43CopyImage implements DepthCopyStrategy {
        ...
        @Override
        public void copy(GlFramebuffer sourceFb, int sourceTexture, GlFramebuffer destFb, int destTexture, int width, int height) {
            GL43C.glCopyImageSubData(sourceTexture, GL43C.GL_TEXTURE_2D, 0, 0, 0, 0, destTexture, GL43C.GL_TEXTURE_2D, 0, 0, 0, 0, width, height, 1);
        }
    }
     ...
     *
     * @param destFb
     *            The destination framebuffer. If {@link #needsDestFramebuffer()} returns false, then this param
