    ...
    public Vector3i getWorkGroups(float width, float height) {
        if (cachedWidth != width || cachedHeight != height || cachedWorkGroups == null) {
            ...
            if (this.absoluteWorkGroups != null) {
                ...
                this.cachedWorkGroups = new Vector3i((int) Math.ceil(Math.ceil((width * relativeWorkGroups.x)) / localSize[0]), (int) Math.ceil(Math.ceil(
                    (height * relativeWorkGroups.y)) / localSize[1]), 1);
            } else {
