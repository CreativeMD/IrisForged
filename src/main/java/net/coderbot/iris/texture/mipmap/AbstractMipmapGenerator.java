            ...
            for (int level = 1; level <= mipLevel; ++level) {
                ...
                for (int x = 0; x < width; ++x) {
                    for (int y = 0; y < height; ++y) {
                        mipmap.setPixelRGBA(x, y, blend(prevMipmap.getPixelRGBA(x * 2 + 0, y * 2 + 0), prevMipmap.getPixelRGBA(x * 2 + 1, y * 2 + 0), prevMipmap.getPixelRGBA(
                            x * 2 + 0, y * 2 + 1), prevMipmap.getPixelRGBA(x * 2 + 1, y * 2 + 1)));
                    }
                }
