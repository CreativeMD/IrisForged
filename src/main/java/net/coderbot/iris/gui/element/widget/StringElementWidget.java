...
public class StringElementWidget extends BaseOptionElementWidget<OptionMenuStringOptionElement> {
    ...
    public void init(ShaderPackScreen screen, NavigationController navigation) {
        ...
        this.prefix = Component.literal(Language.getInstance().has("prefix." + this.option.getName()) ? Language.getInstance().getOrDefault("prefix." + this.option
                .getName()) : "");
        this.suffix = Component.literal(Language.getInstance().has("suffix." + this.option.getName()) ? Language.getInstance().getOrDefault("suffix." + this.option
                .getName()) : "");
    ...
    protected Component createValueLabel() {
        return prefix.copy().append(GuiUtil.translateOrDefault(Component.literal(getValue()), "value." + this.option.getName() + "." + getValue())).append(suffix).withStyle(
            style -> style.withColor(TextColor.fromRgb(0x6688ff)));
    }
