...
public abstract class BaseOptionElementWidget<T extends OptionMenuElement> extends CommentedElementWidget<T> {
    ...
    protected final void renderOptionWithValue(GuiGraphics guiGraphics, boolean hovered, float sliderPosition, int sliderWidth) {
        ...
        // Draw the value box
        GuiUtil.drawButton(guiGraphics, bounds.getBoundInDirection(ScreenDirection.RIGHT) - (this.valueSectionWidth + 2), bounds.position().y() + 2, this.valueSectionWidth, bounds
                .height() - 4, false, true);
        ...
        guiGraphics.drawString(font, this.valueLabel, (bounds.getBoundInDirection(ScreenDirection.RIGHT) - 2) - (int) (this.valueSectionWidth * 0.5) - (int) (font.width(
            this.valueLabel) * 0.5), bounds.position().y() + 7, 0xFFFFFF);
    }
    ...
    public boolean keyPressed(int keycode, int scancode, int modifiers) {
        if (keycode == InputConstants.KEY_RETURN) {
            boolean refresh = Screen.hasControlDown() ? applyOriginalValue() : (Screen.hasShiftDown() ? applyPreviousValue() : applyNextValue());
