    ...
    public static ImmutableList<ClearPass> createClearPasses(RenderTargets renderTargets, boolean fullClear, PackRenderTargetDirectives renderTargetDirectives) {
        ...
        renderTargetDirectives.getRenderTargetSettings().forEach((bufferI, settings) -> {
            ...
            if (fullClear || settings.shouldClear()) {
                ...
                clearByColor.computeIfAbsent(new Vector2i(target.getWidth(), target.getHeight()), size -> new HashMap<>()).computeIfAbsent(
                    new ClearPassInformation(clearColor, target.getWidth(), target.getHeight()), color -> new IntArrayList()).add(buffer);
            }
        });
        ...
        clearByColor.forEach((passSize, vector4fIntListMap) -> {
            vector4fIntListMap.forEach((clearInfo, buffers) -> {
                ...
                while (startIndex < buffers.size()) {
                    ...
                    clearPasses.add(new ClearPass(clearInfo.getColor(), clearInfo::getWidth, clearInfo::getHeight, renderTargets.createClearFramebuffer(true,
                        clearBuffers), GL21C.GL_COLOR_BUFFER_BIT));
                    
                    clearPasses.add(new ClearPass(clearInfo.getColor(), clearInfo::getWidth, clearInfo::getHeight, renderTargets.createClearFramebuffer(false,
                        clearBuffers), GL21C.GL_COLOR_BUFFER_BIT));
                }
            });
        });
    ...
    public static ImmutableList<ClearPass> createShadowClearPasses(ShadowRenderTargets renderTargets, boolean fullClear, PackShadowDirectives renderTargetDirectives) {
        ...
        clearByColor.forEach((clearColor, buffers) -> {
            ...
            while (startIndex < buffers.size()) {
                ...
                // No need to clear the depth buffer, since we're using Minecraft's depth buffer.
                clearPasses.add(new ClearPass(clearColor, renderTargets::getResolution, renderTargets::getResolution, renderTargets.createFramebufferWritingToAlt(
                    clearBuffers), GL21C.GL_COLOR_BUFFER_BIT));
                
                clearPasses.add(new ClearPass(clearColor, renderTargets::getResolution, renderTargets::getResolution, renderTargets.createFramebufferWritingToMain(
                    clearBuffers), GL21C.GL_COLOR_BUFFER_BIT));
            }
        });
