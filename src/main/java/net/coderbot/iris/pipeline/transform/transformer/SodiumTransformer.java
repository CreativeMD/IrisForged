...
public class SodiumTransformer {
    public static void transform(ASTParser t, TranslationUnit tree, Root root, SodiumParameters parameters) {
        ...
        if (parameters.type.glShaderType == ShaderType.VERTEX) {
            ...
            tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, "uniform mat4 iris_ProjectionMatrix;", "uniform mat4 iris_ModelViewMatrix;",
    ...
    public static void injectVertInit(ASTParser t, TranslationUnit tree, Root root, SodiumParameters parameters) {
        ...
        tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_FUNCTIONS,
            ...
            "vec3 _vert_position;", "vec2 _vert_tex_diffuse_coord;", "ivec2 _vert_tex_light_coord;", "vec4 _vert_color;", "uint _draw_id;",
            "const uint MATERIAL_USE_MIP_OFFSET = 0u;",
            "float _material_mip_bias(uint material) {\n" + "    return ((material >> MATERIAL_USE_MIP_OFFSET) & 1u) != 0u ? 0.0f : -4.0f;\n" + "}",
            "void _vert_init() {" + "_vert_position = (vec3(a_PosId.xyz) * 0.00048828125 + -8.0" + ");" + "_vert_tex_diffuse_coord = (a_TexCoord * 1.52587891E-5);" + "_vert_tex_light_coord = a_LightCoord;" + "_vert_color = " + separateAo + ";" + "_draw_id = (a_PosId.w >> 8u) & 0xFFu; }",
    ...
    public static void replaceMidTexCoord(ASTParser t, TranslationUnit tree, Root root, float textureScale) {
        ...
        for (Identifier id : root.identifierIndex.get("mc_midTexCoord")) {
            ...
            DeclarationExternalDeclaration declaration = (DeclarationExternalDeclaration) initDeclaration.getAncestor(1, 0, DeclarationExternalDeclaration.class::isInstance);
            if (declaration == null) {
                continue;
            }
