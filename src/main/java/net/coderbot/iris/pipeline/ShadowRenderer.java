...
public class ShadowRenderer {
    ...
    private void configureSamplingSettings(PackShadowDirectives shadowDirectives) {
        final ImmutableList<PackShadowDirectives.DepthSamplingSettings> depthSamplingSettings = shadowDirectives.getDepthSamplingSettings();
        
        final Int2ObjectMap<PackShadowDirectives.SamplingSettings> colorSamplingSettings = shadowDirectives.getColorSamplingSettings();
    ...
    private FrustumHolder createShadowFrustum(float renderMultiplier, FrustumHolder holder) {
        ...
        if ((packCullingState == ShadowCullState.DEFAULT && packHasVoxelization) || packCullingState == ShadowCullState.DISTANCE) {
            ...
            if (isReversed && renderMultiplier < 0)
            ...
            if (isReversed) {
                return holder.setInfo(new ReversedAdvancedShadowCullingFrustum(CapturedRenderingState.INSTANCE.getGbufferModelView(), CapturedRenderingState.INSTANCE
                        .getGbufferProjection(), shadowLightVectorFromOrigin, boxCuller), distanceInfo, cullingInfo);
            } else {
                return holder.setInfo(new AdvancedShadowCullingFrustum(CapturedRenderingState.INSTANCE.getGbufferModelView(), CapturedRenderingState.INSTANCE
                        .getGbufferProjection(), shadowLightVectorFromOrigin, boxCuller), distanceInfo, cullingInfo);
            }
        }
    ...
    public void renderShadows(LevelRendererAccessor levelRenderer, Camera playerCamera) {
        ...
        if (shouldRenderBlockEntities) {
            renderedShadowBlockEntities = ShadowRenderingState.renderBlockEntities(this, bufferSource, modelView, playerCamera, cameraX, cameraY, cameraZ, tickDelta,
                hasEntityFrustum);
        }
    ...
    public void addDebugText(List<String> messages) {
        ...
        if (Iris.getIrisConfig().areDebugOptionsEnabled()) {
            ...
            messages.add(
                "[" + Iris.MODNAME + "] Shadow Terrain: " + debugStringTerrain + (shouldRenderTerrain ? "" : " (no terrain) ") + (shouldRenderTranslucent ? "" : "(no translucent)"));
