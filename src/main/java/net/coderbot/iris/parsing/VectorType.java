...
abstract public class VectorType extends Type.ObjectType {
    ...
    public static final ArrayVector[] AllArrayVectorTypes = Stream.of(Type.Int, Type.Boolean).flatMap(type -> IntStream.rangeClosed(2, 4).mapToObj(i -> new ArrayVector(type, i)))
            .toArray(ArrayVector[]::new);
    
    public static final VectorType[] AllVectorTypes = Arrays.stream(Type.AllPrimitives).flatMap(type -> IntStream.rangeClosed(2, 4).mapToObj(i -> VectorType.of(type, i))).toArray(
        VectorType[]::new);
