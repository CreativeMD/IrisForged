...
public class IrisChunkProgramOverrides {
    ...
    private GlProgram<IrisChunkShaderInterface> createShader(IrisTerrainPass pass, SodiumTerrainPipeline pipeline, ChunkVertexType vertexType) {
        ...
        try {
            ...
            return builder.attachShader(vertShader).attachShader(fragShader)
                    // The following 4 attributes are part of Sodium
                    .bindAttribute("a_PosId", IrisChunkShaderBindingPoints.ATTRIBUTE_POSITION_ID).bindAttribute("a_Color", IrisChunkShaderBindingPoints.ATTRIBUTE_COLOR)
                    .bindAttribute("a_TexCoord", IrisChunkShaderBindingPoints.ATTRIBUTE_BLOCK_TEXTURE).bindAttribute("a_LightCoord",
                        IrisChunkShaderBindingPoints.ATTRIBUTE_LIGHT_TEXTURE).bindAttribute("mc_Entity", IrisChunkShaderBindingPoints.BLOCK_ID).bindAttribute("mc_midTexCoord",
                            IrisChunkShaderBindingPoints.MID_TEX_COORD).bindAttribute("at_tangent", IrisChunkShaderBindingPoints.TANGENT).bindAttribute("iris_Normal",
                                IrisChunkShaderBindingPoints.NORMAL).bindAttribute("at_midBlock", IrisChunkShaderBindingPoints.MID_BLOCK).link((shader) -> {
                                    ...
                                    return new IrisChunkShaderInterface(handle, contextExt, pipeline, new ChunkShaderOptions(ChunkFogMode.SMOOTH, pass
                                            .toTerrainPass()), pass == IrisTerrainPass.SHADOW || pass == IrisTerrainPass.SHADOW_CUTOUT, blendOverride, bufferOverrides, alpha, pipeline
                                                    .getCustomUniforms());
                                });
        } finally {
            vertShader.delete();
