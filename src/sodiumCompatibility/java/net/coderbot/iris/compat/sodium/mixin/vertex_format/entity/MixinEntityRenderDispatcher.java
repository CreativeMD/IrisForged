...
public class MixinEntityRenderDispatcher {
    ...
    private static void renderShadowPart(PoseStack.Pose matrices, VertexConsumer vertices, float radius, float alpha, float minX, float maxX, float minY, float minZ, float maxZ) {
        ...
        if (extended) {
            tangent = getTangent(normal, minX, minY, minZ, u1, v1, minX, minY, maxZ, u1, v2, maxX, minY, maxZ, u2, v2);
        }
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VertexBufferWriter.of(vertices).push(stack, buffer, 4, extended ? EntityVertex.FORMAT : ModelVertex.FORMAT);
        }
    }
